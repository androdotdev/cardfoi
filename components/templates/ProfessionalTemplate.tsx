...
export default function ProfessionalTemplate({ card }: { card: UserCard }) {
  ...
  return (
    <main className="card-theme-root min-h-screen bg-surface px-4 py-16">
      <div className="mx-auto max-w-4xl">
        {/* Header */}
        <motion.header
          className="border-b border-muted pb-8 text-center"
        ...
        >
          {card.avatar ? (
            ...
            <div className="mx-auto mb-4 flex h-24 w-24 items-center justify-center rounded-full bg-muted text-3xl font-bold text-content">
              {card.name?.slice(0, 1)}
            </div>
          )}
          ...
          <p className="mx-auto mt-3 max-w-xl text-content-60">
            {card.description}
          </p>
          <a
        ...
        {works.length > 0 && (
          <section className="mt-10">
            <h2 className="mb-6 text-center text-sm font-semibold uppercase tracking-widest text-content-40">
              Selected Work
            </h2>
            <div className="space-y-8">
              {works.map((work, i) => (
                <motion.article
                  key={work.id}
                  className="flex flex-col gap-4 rounded-2xl border border-muted bg-surface p-6 sm:flex-row sm:items-center"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {work.description && (
                      <p className="mt-1 text-content-55">
                        {work.description}
                      </p>
                    )}
                    {work.type === "link" && (
